...
def concise_policy_answer(
    ...
    *,
    ...
    for _, _, part, title in candidates:
        normalized = part.rstrip("\uff1b\u3002")
        if normalized and normalized not in selected_texts:
            selected.append((title, normalized))
    ...
    if not selected:
        return intro
    prefix = intro.rstrip("\uff1a:\uff0c,\u3002")
    rendered = [f"《{title}》\uff1a{part}" if title else part for title, part in selected]
