...
5. Run all four k6 scenarios and one dependency-loss chaos drill. At minimum cover `public-catalog`, `user-workspace`, `messaging-read`, `merchant-workspace`, `admin-workspace` and `mixed-read`; pass credentials and stable resource IDs through environment variables, never command-line literals or committed files. Capacity is accepted only with at least 30% headroom for connections, disk, queues, workers, Agent concurrency, and model spend. Every report must include the generated manifest binding the result to Commit SHA, runtime Build SHA, workload, dataset label and machine profile.
