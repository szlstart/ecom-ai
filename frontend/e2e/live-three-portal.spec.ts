...
test.describe('LIVE-THREE-PORTAL connected acceptance', () => {
  ...
  test('LIVE-AGENT-BROWSER runs all three controlled agents with auditable trace summaries', async ({ browser, isMobile }) => {
    ...
    const consumer = await consumerContext.newPage()
    await loginConsumer(consumer, data.consumer_username)
    await consumer.getByRole('button', { name: '消息', exact: true }).click()
    const consumerDialog = consumer.getByRole('dialog', { name: '用户消息中心' })
    await consumerDialog.getByPlaceholder('输入消息…').fill('请介绍验收商品，并说明你使用了什么可信依据。')
    await consumerDialog.getByRole('button', { name: '发送', exact: true }).click()
    await expect(consumerDialog.getByText('智能客服')).toBeVisible({ timeout: 20_000 })
    ...
    const merchant = await merchantContext.newPage()
    await loginMerchant(merchant, data.merchant_username)
    await merchant.getByRole('button', { name: /消息/ }).click()
    const merchantDialog = merchant.getByRole('dialog', { name: '商家消息中心' })
    ...
    const administrator = await adminContext.newPage()
    await loginAdministrator(administrator, data.administrator_username)
    await administrator.getByRole('button', { name: '打开消息中心' }).click()
    const adminDialog = administrator.getByRole('dialog', { name: '管理端消息中心' })
