...
const loading = ref(true), error = ref(''), attempts = ref(0)
  ...
  <section class="content-section payment-bridge-page">
    <PageState :loading="loading" :error="error" :empty="!payment" empty-title="支付单不可用" empty-message="请返回订单列表重试。" @retry="load">
      <article v-if="payment" class="card" aria-live="polite">
        ...
        <div class="actions"><RouterLink :to="`/pay/${payment.trade_order_id}`">返回收银台</RouterLink><RouterLink to="/me/orders?view=all">查看订单</RouterLink></div>
      </article>
    </PageState>
  </section>
</template>
