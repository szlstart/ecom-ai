...
async function load() {
  ...
  try {
    items.value = (await adminGet<{ items: AdminInventory[] }>(`/admin/inventories${adminQuery({ q: q.value.trim(), store_id: storeId.value.trim(), limit: 100 })}`, requireAdminToken(auth.accessToken))).data.items
...
<template>
  <section class="admin-page-stack">
    ...
    <form class="filter-bar" @submit.prevent="load"><label>搜索 SKU / 商品<input v-model="q" maxlength="100" /></label><label>店铺公开 ID<input v-model="storeId" maxlength="40" placeholder="sto_…" /></label><button>查询</button></form>
    ...
    <div class="admin-split inventory-layout">
      <PageState :loading="loading" :error="''" :empty="!loading && items.length === 0" empty-title="没有匹配的库存">
        <div class="table-wrap"><table><thead><tr><th>SKU / 商品</th><th>店铺</th><th>现有/预占</th><th>可售</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="item in items" :key="item.sku_id" :class="{ selected: selected?.sku_id === item.sku_id }"><td><strong>{{ item.sku_name }}</strong><small>{{ item.product_name }} · {{ item.sku_id }}</small></td><td>{{ item.store_name }}</td><td>{{ item.on_hand_quantity }} / {{ item.reserved_quantity }}</td><td>{{ item.available_quantity }}</td><td><span class="badge">{{ item.status }}</span></td><td><button type="button" class="secondary small" @click="choose(item)">调整</button></td></tr></tbody></table></div>
      </PageState>
