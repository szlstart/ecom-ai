...
async function load() {
  ...
  if (isNew.value) { product.value = null; loading.value = false; return }
  try {
...
async function saveBasic() {
  if (isNew.value) {
    ...
    try {
      ...
      await router.replace(`/admin/products/${created.product_id}`)
    } catch (cause) { error.value = errorMessage(cause) }
    finally { saving.value = false }
    return
  }
...
<template>
  <section class="admin-page-stack"><header class="page-heading"><div><p class="eyebrow">商品管理</p><h1>{{ isNew ? '新建商品' : product?.product_name || '商品详情' }}</h1><p class="muted">{{ isNew ? '先建立商品草稿，再补齐 SKU、图片、属性、履约和安全内容版本。' : `${productId} · ${product?.store_name || ''}` }}</p></div><RouterLink to="/admin/products">返回商品列表</RouterLink></header><p v-if="message" class="alert success" aria-live="polite">{{ message }}</p><PageState :loading="loading" :error="error" :empty="!loading && !isNew && !product" empty-title="商品不存在" @retry="load"><template v-if="isNew || product"><nav v-if="!isNew" class="tab-list" aria-label="商品编辑分区"><button v-for="entry in ([['basic','基础信息'],['skus','SKU'],['images','图片'],['attributes','属性'],['fulfillment','履约'],['content','详情内容'],['faqs','FAQ'],['publication','发布检查']] as const)" :key="entry[0]" type="button" :class="{ active: section === entry[0] }" @click="section = entry[0]">{{ entry[1] }}</button></nav>
      <form v-if="isNew || section === 'basic'" class="card admin-editor wide-editor" @submit.prevent="saveBasic"><h2>基础信息</h2><label>店铺公开 ID<input v-model.trim="basic.store_id" required maxlength="40" list="store-options" :disabled="!isNew" placeholder="sto_…" /><datalist id="store-options"><option v-for="item in stores" :key="item.store_id" :value="item.store_id">{{ item.store_name }}</option></datalist></label><div class="field-grid"><label>平台分类<select v-model="basic.category_id" required><option value="">请选择</option><option v-for="item in flatCategories" :key="item.category_id" :value="item.category_id">{{ '—'.repeat(Math.max(0, item.level - 1)) }} {{ item.category_name }}</option></select></label><label>品牌<select v-model="basic.brand_id"><option value="">无品牌</option><option v-for="item in brands" :key="item.brand_id" :value="item.brand_id">{{ item.brand_name }}</option></select></label></div><label>商品名称<input v-model.trim="basic.product_name" required maxlength="255" /></label><label>副标题<input v-model.trim="basic.subtitle" maxlength="500" /></label><label>简述<textarea v-model.trim="basic.description" maxlength="2000" /></label><button :disabled="saving">{{ isNew ? '创建商品草稿' : '保存基础信息' }}</button></form>
