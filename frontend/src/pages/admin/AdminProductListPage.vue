...
<template><section class="admin-page-stack admin-product-catalog-page">
  <header class="admin-entity-hero"><div><p class="eyebrow">PRODUCT GOVERNANCE</p><h1>全平台商品</h1><p>按店铺查看商品、审核发布状态与库存健康度，异常商品一眼可见。</p></div><div class="actions"><RouterLink v-if="auth.has('products:create')" class="button-link" to="/admin/products/new">＋ 新建商品</RouterLink><RouterLink v-if="auth.has('products:create')" class="button-link secondary" to="/admin/products/import">批量导入</RouterLink></div></header>
