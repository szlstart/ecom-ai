    ...
    <PageState :loading="loading" :error="error" :empty="!loading && !store" empty-title="店铺不存在" @retry="load">
      <template v-if="store">
        ...
        <section v-if="section === 'products'" class="admin-store-panel">
          <header><div><p class="eyebrow">店铺商品</p><h2>该店铺的商品</h2><p>点击商品后直接进入与商家端一致的所见即所得编辑页；操作身份仍是超级管理员并完整留痕。</p></div><RouterLink v-if="auth.has('products:create')" class="button-link" :to="{ path: `/admin/stores/${storeId}/products/new`, query: { return_to: route.fullPath } }">＋ 为该店铺新增商品</RouterLink></header>
        ...
        <section v-else class="admin-store-panel admin-store-orders-panel">
          <header><div><p class="eyebrow">店铺订单</p><h2>该店铺的订单</h2><p>超级管理员负责监管和异常处置；页面中的“顾客”与“店铺”角色保持真实关系。</p></div></header>
