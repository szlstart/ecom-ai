...
async function submit() {
  ...
  try {
    ...
    auth.clear()
    await router.replace('/?accountDeleted=1')
...
<template>
  <section class="narrow-page"><p class="eyebrow danger-text">不可恢复</p><h1>账号注销</h1><div class="alert warning"><strong>注销不是停用，而是立即从数据库删除账号。</strong><p>收货地址、收藏、余额和非交易数据会一并永久删除，不设冷静期。存在历史订单时，为保护双方交易记录，系统会阻止直接删除并提示联系平台客服。</p></div><p v-if="error" class="alert error" role="alert">{{ error }}</p><form class="card" @submit.prevent="submit"><label>输入 DELETE_MY_ACCOUNT 以确认<input v-model="confirmation" required autocomplete="off" /></label><button class="danger" :disabled="pending || confirmation !== 'DELETE_MY_ACCOUNT'">{{ pending ? '正在永久删除…' : '永久注销账号' }}</button></form></section>
</template>
