...
<template>
  <section class="storefront-stack">
    <header class="page-heading"><div><p class="eyebrow">我的收藏</p><h1>收藏的商品</h1></div><RouterLink to="/me">返回我的</RouterLink></header>
    <PageState :loading="loading" :error="error" :empty="!loading && !error && products.length === 0" empty-title="还没有收藏商品" empty-detail="浏览商品时点击收藏，之后可以在这里快速找到。" @retry="load">
      ...
      <div class="favorite-grid"><div v-for="product in products" :key="product.product_id" class="favorite-item"><ProductCard :product="product" return-to="/me/favorites/products" /><button type="button" class="secondary small" :disabled="removing === product.product_id" @click="remove(product)">移除收藏</button></div></div>
    </PageState>
  </section>
</template>
